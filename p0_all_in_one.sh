...
echo "================================================================"
echo "  P0 All-in-One v5: CED验证 (${NUM_GPUS}卡并行)"
echo "================================================================"

# ============================================================
# STEP 0: CUDA修复（必须在任何python之前！）
# ============================================================
echo ""
echo "[0/4] CUDA库路径修复..."

# 把torch自带的nvidia库路径加到最前面，覆盖系统的旧版本
...
# ============================================================
# STEP 1e: 防止accelerate等把huggingface_hub降级
# ============================================================
echo "  [1e] 确认huggingface_hub版本..."
HF_VER=$(python -c "import huggingface_hub; print(huggingface_hub.__version__)" 2>/dev/null || echo "missing")
echo "  当前: huggingface_hub=$HF_VER"
if [[ "$HF_VER" != 1.5.0 ]]; then
    echo "  被其他包降级了，强制恢复1.5.0..."
    pip install --no-index --no-cache-dir --no-deps --force-reinstall \
        --no-warn-script-location \
        "$WHEELS/huggingface_hub-1.5.0-py3-none-any.whl" 2>/dev/null || true
    HF_VER=$(python -c "import huggingface_hub; print(huggingface_hub.__version__)" 2>/dev/null)
    echo "  恢复后: huggingface_hub=$HF_VER"
fi
...
" || {
    ...
    pip show transformers huggingface_hub tokenizers 2>/dev/null | grep -E '^(Name|Version)'
    exit 1
}
...
P0 All-in-One
  ...
  --mode worker   : P0-b worker（单卡处理一个shard）
  --mode analyze  : 合并shard + 分析出图
"""
...
def js(p,q):
    p,q=_p(p),_p(q); m=.5*(p+q); return float(.5*entropy(p,m,base=2)+.5*entropy(q,m,base=2))
def kl(p,q): return float(entropy(_p(p),_p(q),base=2))
def H(p): return float(entropy(_p(p),base=2))
...
def mets(ol,cl,lams):
    p,q=_sm(ol),_sm(cl); j=js(p,q); k_=kl(p,q); ho=H(p); hc=H(q); ep=max(0.,hc-ho)
    r={"js":j,"kl":k_,"h_orig":ho,"h_cf":hc,"ep":ep}
    for l in lams:
        r[f"ced_{l:.2f}"]=j+l*ep; r[f"ced_abs_{l:.2f}"]=j+l*abs(hc-ho); r[f"ced_hcf_{l:.2f}"]=j+l*hc
    return r

# ── Hook ──
class VTHook:
    def __init__(self): self.cap=None; self._m=None; self._h=None
    def reg(self,model): self._h=model.visual.register_forward_hook(self._fn); return self
    def rm(self):
        if self._h: self._h.remove()
    def reset(self): self.cap=None; self._m=None
    def set_rep(self,m): self._m=m
    def _fn(self,mod,inp,out):
        if self._m is None: self.cap=out.detach().clone(); return out
...
def bbox2idx(bb,iw,ih,gh,gw):
    x,y,w,h=bb; cs,ce=max(0,int(x/iw*gw)),min(gw,int(np.ceil((x+w)/iw*gw)))
...
def run_probe(model,proc,dev):
    ...
    if anns:
        ann=max(anns,key=lambda a:a["bbox"][2]*a["bbox"][3]); obj=cm[ann["category_id"]]; bb=ann["bbox"]
    ...
    h2.remove()
    ol_=orig.logits[0,-1].cpu().float().numpy(); cl_=cf.logits[0,-1].cpu().float().numpy()
    js_obj=js(_sm(ol_),_sm(cl_))

    ci=[r*gw+c for r in range(min(2,gh)) for c in range(min(2,gw))]
    cs=sorted(set(range(vf.shape[0]))-set(ci)); cr=vf[cs].mean(0) if cs else vf.mean(0)
    cf2=vf.clone()
    for i in ci:
...
def load_samples(n=400):
    with open(COCO_ANN) as f: coco=json.load(f)
    cm={c["id"]:c["name"] for c in coco["categories"]}; im={i["id"]:i for i in coco["images"]}
    ia=defaultdict(list)
    for a in coco["annotations"]: ia[a["image_id"]].append(a)
    ic={iid:set(a["category_id"] for a in aa) for iid,aa in ia.items()}; ac=set(cm.keys())
    ...
    for iid in ids:
        ...
        if not va: continue
        tgt=max(va,key=lambda a:a["bbox"][2]*a["bbox"][3]); cat=cm[tgt["category_id"]]; bb=tgt["bbox"]; abb=[a["bbox"] for a in aa]
        samps.append(dict(task="existence",path=p,w=w,h=h,obj=cat,bb=bb,abb=abb,gt=1,q=f"Is there a {cat} in this image? Answer yes or no."))
        absent=ac-ic.get(iid,set())
        if absent:
            an=cm[random.choice(list(absent))]
            samps.append(dict(task="existence",path=p,w=w,h=h,obj=an,bb=bb,abb=abb,gt=0,q=f"Is there a {an} in this image? Answer yes or no."))
        tmpl,fn=random.choice(SPATIAL); gs=1 if fn(bb,w,h) else 0
        samps.append(dict(task="spatial",path=p,w=w,h=h,obj=cat,bb=bb,abb=abb,gt=gs,q=tmpl.format(o=cat)+" Answer yes or no."))
        ...
        samps.append(dict(task="counting",path=p,w=w,h=h,obj=cat,bb=bb,abb=abb,gt=1 if ask==cnt else 0,q=f"Are there exactly {ask} {cat}(s) in this image? Answer yes or no."))
    return samps
...
def proc_one(s,model,proc,hook,dev,layers,lams):
    ...
    vf=hook.cap; si=sorted(set(range(vf.shape[0]))-set(ti))
    rp=vf[si].mean(0) if si else vf.mean(0)
    hook.set_rep({i:rp for i in ti})
    with torch.no_grad(): cf=model(**inp,output_hidden_states=True,return_dict=True)
    hook.reset()
    ol=orig.logits[0,-1].cpu().float().numpy(); cl=cf.logits[0,-1].cpu().float().numpy()
    m=mets(ol,cl,lams)
    m["cosine"]=float(cosine_dist(orig.hidden_states[-1][0,-1].cpu().float().numpy(),cf.hidden_states[-1][0,-1].cpu().float().numpy()))
    for lay in layers:
        if lay<len(orig.hidden_states): m[f"L{lay}_js"]=hid_js(orig.hidden_states[lay][0,-1],cf.hidden_states[lay][0,-1])
    r={"task":s["task"],"obj":s["obj"],"gt":gt,"pred":pred,"beh":beh,"n_tgt":len(ti),"n_tot":gh*gw,**m}

    if beh=="correct_positive":
        ci=ctridx(s["abb"],s["w"],s["h"],gh,gw,max(4,len(ti)))
        csurr=sorted(set(range(vf.shape[0]))-set(ci)); cr=vf[csurr].mean(0) if csurr else vf.mean(0)
        hook.reset()
        ...
        hook.reset()
        cm_=mets(ol,co.logits[0,-1].cpu().float().numpy(),lams)
        r["ctrl_js"]=cm_["js"]
        for l in lams: r[f"ctrl_ced_{l:.2f}"]=cm_[f"ced_{l:.2f}"]
    return r
...
# ════════════════════════════════════════
def run_worker(args):
    sid,ns=args.shard_id,args.num_shards; lams=args.lambda_e
    print(f"[W{sid}/{ns}] GPU={os.environ.get('CUDA_VISIBLE_DEVICES','?')}")
    dev=torch.device("cuda:0")
    from transformers import Qwen3VLForConditionalGeneration,AutoProcessor
    model=Qwen3VLForConditionalGeneration.from_pretrained(MODEL_PATH,torch_dtype=torch.bfloat16,attn_implementation="sdpa").to(dev).eval()
    ...
    for s in tqdm(my_s,desc=f"W{sid}",position=sid):
        r=proc_one(s,model,proc,hook,dev,layers,lams)
        if r: results.append(r); bc[r["beh"]]+=1
    hook.rm(); df=pd.DataFrame(results)
    out=f"{RESULT_DIR}/p0b_shard_{sid}.csv"; df.to_csv(out,index=False)
    print(f"[W{sid}] 完成: {len(df)}条, {dict(bc)}")
...
def run_analyze(args):
    ...
    if not files: print("没有shard文件！"); sys.exit(1)
    df=pd.concat([pd.read_csv(f) for f in files],ignore_index=True)
    df.to_csv(f"{RESULT_DIR}/p0b_results.csv",index=False)
    print(f"\n合并{len(files)}个shard, 共{len(df)}条")
    ...
    for l in lams:
        c=f"ced_{l:.2f}"
        if c in sub:
            a=safe_auc(sub["label"],sub[c]); tag=" ★" if a>ba else ""
            print(f"  B. CED(λ={l:.2f}):       AUC={a:.4f}{tag}")
            if a>ba: ba,bn=a,c
    for c,nm in [("ced_hcf_0.10","C. JS+λH^cf"),("ced_abs_0.10","D. JS+λ|ΔH|")]:
        if c in sub:
            a=safe_auc(sub["label"],sub[c]); print(f"  {nm:23s}: AUC={a:.4f}")
            if a>ba: ba,bn=a,c
    a=safe_auc(sub["label"],sub["kl"]); print(f"  E. KL:                 AUC={a:.4f}")
    if a>ba: ba,bn=a,"kl"
    if "cosine" in sub:
        a=safe_auc(sub["label"],sub["cosine"]); print(f"  F. Cosine:             AUC={a:.4f}")
        if a>ba: ba,bn=a,"cosine"
    print(f"\n  → 最优: {bn}, AUC={ba:.4f}")
    ...
    for lay in layers:
        c=f"L{lay}_js"
        if c in sub and sub[c].notna().sum()>10: print(f"  Layer {lay}: AUC={safe_auc(sub['label'],sub[c]):.4f}")
    ...
    for b in ["correct_positive","hallucination","correct_negative"]:
        s=df[df["beh"]==b]
        if len(s)>0: print(f"  {b:20s}: H(P)={s['h_orig'].mean():.3f}, H^cf={s['h_cf'].mean():.3f}, Δ+={s['ep'].mean():.4f}")
    ...
    ax=axes[1,0]
    if sub["label"].nunique()>1:
        fpr,tpr,_=roc_curve(sub["label"],sub["js"]); ax.plot(fpr,tpr,lw=2,label=f"JS AUC={safe_auc(sub['label'],sub['js']):.3f}")
        ax.plot([0,1],[0,1],"k--",alpha=.3); ax.set_title("ROC"); ax.legend()
    ax=axes[1,1]
    if "ctrl_js" in cp.columns and cp["ctrl_js"].notna().sum()>5:
        ax.boxplot([cp["js"].dropna(),cp["ctrl_js"].dropna()],labels=["Object","Control"]); ax.set_ylabel("JS"); ax.set_title("Object vs Control")
    ax=axes[1,2]
    ta={}
    for task in ["existence","spatial","counting"]:
        t=df[df["task"]==task]; c1=t[t["beh"]=="correct_positive"]; c2=t[t["beh"]=="hallucination"]
        if len(c1)>=3 and len(c2)>=3:
            s2=pd.concat([c1.assign(label=1),c2.assign(label=0)]); ta[task]=safe_auc(s2["label"],s2["js"])
    if ta:
    ...
    print("\n"+"="*60)
    if ba>.85: print(f"✅ P0-b PASSED: AUC={ba:.4f} > 0.85 ({bn})\n   → Phase 1!")
    elif ba>.75: print(f"⚠️  P0-b MARGINAL: AUC={ba:.4f}")
    else: print(f"❌ P0-b FAILED: AUC={ba:.4f}")
    print("="*60)
...
if __name__=="__main__":
    ...
    elif args.mode=="worker": run_worker(args)
    elif args.mode=="analyze": run_analyze(args)
PYTHON_EOF

echo "  ✓ $CODE_DIR/p0_all.py"
...
[ $FAIL -ne 0 ] && { echo "部分worker失败"; exit 1; }
echo "  ✓ 全部完成"

# ── 4c: 合并分析 ──
echo ""
echo "[4c] 合并 + 分析..."
python p0_all.py --mode analyze --seed 42 2>&1 | tee "$LOG_DIR/p0b_analyze.log"
